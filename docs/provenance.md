...
| tauri 2.11.5 crate source: `build.rs`; tauri-build 2.6.3 crate source: `src/lib.rs`, `src/windows-app-manifest.xml` | 2026-09-13 | Why a Tauri test binary exits with STATUS_ENTRYPOINT_NOT_FOUND on Windows, and how Tauri's own tests link the manifest | Apache-2.0 OR MIT | The lab's `windows-test-manifest.xml` is the standard Common Controls 6 dependency that tauri-build's default manifest also declares | Link only |
