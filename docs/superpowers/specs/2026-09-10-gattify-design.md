...
- The prerelease publishes. The owner runs `cargo publish` and `npm publish` for `0.1.0-alpha.0` and `0.1.0-alpha.1`, as `docs/release-checklist.md` states.
...
2. Publish `0.1.0-alpha.0` by hand to claim the package names. Then configure trusted publishing, as `docs/release-checklist.md` states.
3. After sub-projects 3 and 4, run the lab app on the iPhone and on the Android phone. Test each phone as host and as joiner. Record the results in `docs/platforms/test-results/`.
4. After sub-project 5, publish `0.1.0-alpha.1` by hand.
5. After the phone test passes, set the repository variable `RELEASE_ON_MERGE` to `true`. The next merge into `develop` publishes `0.1.0`.
